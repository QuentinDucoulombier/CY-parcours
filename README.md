...
Date: 2 Juin 2022  
Email: ducoulombi@cy-tech.fr, achillefur@cy-tech.fr, thupingone@cy-tech.fr, fauchernoe@cy-tech.fr

---
## Presentation

En cours de la deuxieme année du cycle ingénieur à CY Tech, les étudiants préparent leur année suivante en choisissant l’option de leur rêve en dernière année.  
Chaque option cependant possède un nombre limité de places, elles ne peuvent pas systématiquement accueillir tous les étudiants souhaitant la faire. Aussi, certains étudiants souhaitent faire une option particulière mais n’ont malheureusement pas les résultats suffisants pour pouvoir réussir dans de bonnes conditions. Dès lors, il a été défini des critères de mérite permettant de classer les étudiants sur un vœu d’option. Chaque étudiant devra alors classer les options disponibles au bout de son parcours par ordre de préférence.  
C'est en répondant a cette problématique qu'a était créé **CY Parcours**, ce site web utilise un algorithme plaçant les étudiants dans chaque option en minimisant les déceptions étudiantes, tout en respectant les critères de mérite de l’option, pour cela **CY Parcours** utilise notamment [l’algorithme des “mariages stables”](https://fr.wikipedia.org/wiki/Algorithme_de_Gale_et_Shapley). Ce site bénificie également d'une interface facile d'utilisation pour tous les types d'utilisateurs (étudiant, responsable administratif ou encore administrateur).
