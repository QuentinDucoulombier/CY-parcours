        ...
        if (($handle = fopen($fichier, "r")) !== FALSE) {
            while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
                ...
                if (($data[2] == $_POST["email"]) && ($data[1] == $_POST["nom"])){  //On verifie si son adresse mail et son nom existe dans la base de donnée
                    ...
                    $hash = password_hash($_POST['password'], PASSWORD_DEFAULT); //on crypte le mot de passe

                    $list = array($_POST['prenom'], $_POST['nom'], $_POST['email'], $_POST['pseudo'], $hash, "eleves", $_SESSION["pp"], $filiere);
                    fputcsv($file, $list, ";");
                    fclose($file);
